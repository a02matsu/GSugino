...
subroutine Make_force(dSdPhi,dSdA,UMAT,Phi,PF,info)
implicit none
...
complex(kind(0d0)) :: dSdPhi_boson_mass(1:dimG,1:num_sites)
complex(kind(0d0)) :: dSdPhi_boson_site(1:dimG,1:num_sites)
complex(kind(0d0)) :: dSdPhi_boson_link(1:dimG,1:num_sites)
...
call Make_bosonic_force_Phi_mass(dSdPhi_boson_mass,Phi)
call Make_bosonic_force_Phi_site(dSdPhi_boson_site,Phi)
call Make_bosonic_force_Phi_link(dSdPhi_boson_link,UMAT,Phi)

!! force for A from boson
call Make_bosonic_force_A_link(dSdA_boson_link,UMAT,Phi)
...
dSdPhi= dSdPhi + dSdPhi_boson_site  ! site part
dSdPhi= dSdPhi + dSdPhi_boson_link   ! link part
...
subroutine Make_bosonic_force_Phi_mass(dSdPhi_boson_mass,Phi)
implicit none

complex(kind(0d0)), intent(in) :: Phi(1:dimG,1:num_sites)
complex(kind(0d0)), intent(out) :: dSdPhi_boson_mass(1:dimG,1:num_sites)
integer :: a,s

dSdPhi_boson_mass=(0d0,0d0)
do a=1,dimG
do s=1,num_sites
  dSdPhi_boson_mass(a,s) = dcmplx(mass_square_phi) * dconjg(Phi(a,s)) &
...
subroutine Make_bosonic_force_Phi_site(dSdPhi_boson_site,Phi)
use SUN_generators, only : make_traceless_matrix_from_modes,trace_MTa
use matrix_functions, only : matrix_commutator
implicit none

complex(kind(0d0)), intent(in) :: Phi(1:dimG,1:num_sites)
complex(kind(0d0)), intent(out) :: dSdPhi_boson_site(1:dimG,1:num_sites)
integer :: a,s
...
do s=1,num_sites
  ...
  call matrix_commutator(comm1,phi_mat,phi_mat,'N','C')

  call matrix_commutator(comm2,phi_mat,comm1,'C','N')
  do a=1,dimG
    call trace_MTa(trace,comm2,a,NMAT)
    dSdPhi_boson_site(a,s)=dcmplx(0.5d0*alpha_s(s))*trace & 
      * cmplx( overall_factor )
      !* cmplx( one_ov_2g2N )
  enddo
enddo
...
subroutine Make_bosonic_force_Phi_link(dSdPhi_boson_link,UMAT,Phi)
use SUN_generators, only : trace_MTa, make_traceless_matrix_from_modes
implicit none

complex(kind(0d0)), intent(in) :: UMAT(1:NMAT,1:NMAT,1:num_links)
complex(kind(0d0)), intent(in) :: Phi(1:dimG,1:num_sites)
complex(kind(0d0)), intent(out) :: dSdPhi_boson_link(1:dimG,1:num_sites)
complex(kind(0d0)) :: dPhi(1:NMAT,1:NMAT)
complex(kind(0d0)) :: tmpmat(1:NMAT,1:NMAT), tmpmat2(1:NMAT,1:NMAT)
complex(kind(0d0)) :: Tr_Ta_Ud_dPhibar_U(1:dimG,1:num_links)
complex(kind(0d0)) :: Tr_Ta_dPhibar(1:dimG,1:num_links)
complex(kind(0d0)) :: trace
integer :: a,s
integer :: l, l_i
integer :: i,j,k
...
! diff( \Phi_s ) for the link l
  call Make_diff_Phi(dPhi,l,UMAT,Phi)
...
do s=1,num_sites
  do i=1,linkorg_to_s(s)%num_
    ...
    do a=1,dimG
      dSdPhi_boson_link(a,s)=dSdPhi_boson_link(a,s) + alpha_l(l_i) * Tr_Ta_Ud_dPhibar_U(a,l_i)
    enddo
  enddo
  ...
  do i=1,linktip_from_s(s)%num_
    ...
    do a=1,dimG
      dSdPhi_boson_link(a,s)=dSdPhi_boson_link(a,s) - alpha_l(l_i) * Tr_Ta_dPhibar(a,l_i)
    enddo
  enddo
enddo
...
subroutine Make_bosonic_force_A_link(dSdA_boson_link,UMAT,Phi)
use SUN_generators, only : trace_MTa, make_traceless_matrix_from_modes
implicit none
