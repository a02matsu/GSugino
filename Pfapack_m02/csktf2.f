...
*  =======
*
*  CSKTF2 computes the factorization of a skew-symmetric matrix A
...
*
*
*  The normal use for CSKTD2 is to compute the U T U^T or L T L^T
...
*  or in the even columns (if UPLU = 'U') are properly computed by CSKTF2.
*
...
*  element not computed by CSKTF2.
*
*  =====================================================================
*
...
*     .. External Subroutines ..
      EXTERNAL           CSCAL, CSWAP, CSKR2, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*     (The column/row K-1 is not affected by the update)
               IF( COLMAX .NE. ZERO ) THEN
                  CALL CSKR2( UPLO, K-2, ONE/A( K-1,K ), A( 1, K ), 1,
     $                 A( 1, K-1 ), 1, A( 1, 1 ), LDA )
...
*     (The column/row K+1 is not affected by the update)
               IF( COLMAX .NE. ZERO .AND. K+2 .LE. N) THEN
                  CALL CSKR2( UPLO, N-K-1, ONE/A( K+1,K ),
