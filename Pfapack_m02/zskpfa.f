      SUBROUTINE ZSKPFA( UPLO, MTHD, N, A, LDA, LOGPF, PHASEPF,
...
*      DOUBLE COMPLEX     PFAFF
      DOUBLE PRECISION   LOGPF
      DOUBLE COMPLEX     PhasePF
...
*            PFAFF = ONE
            ...
            DO 10 I = 1, N-1, 2
*               PFAFF = PFAFF * A( I, I+1 )
               LOGPF = LOGPF + dlog( cdabs( A(I,I+1) ) )
               PHASEPF = PHASEPF * A(I,I+1) / cdabs( A(I,I+1) )

*     Accumulate the determinant of the permutations
*               IF( IWORK( I ) .NE. I ) PFAFF = -PFAFF
               IF( IWORK( I ) .NE. I ) PHASEPF = (-1d0,0d0)*PHASEPF
 10         CONTINUE
...
*            PFAFF = ONE
            ...
            DO 20 I = 1, N-1, 2
*               PFAFF = PFAFF * (-A( I+1, I ))
               LOGPF = LOGPF + dlog( cdabs( A(I+1,I) ) )
               PHASEPF = PHASEPF*(-1d0,0d0)*A(I+1,I)/cdabs(A(I+1,I))

*     Accumulate the determinant of the permutations
*               IF( IWORK( I+1 ) .NE. I+1 ) PFAFF = -PFAFF
               IF( IWORK( I+1 ) .NE. I+1 ) PHASEPF = (-1d0,0d0)*PHASEPF
 20         CONTINUE
...
*     Multiply every other entry on the superdiagonal
            DO 30 I = 1, N-1, 2
*               PFAFF = PFAFF * RWORK( I )
               LOGPF = LOGPF + dlog(dabs(RWORK( I )))
               PhasePF = PhasePF * dcmplx((RWORK( I ))/dabs(RWORK( I )))
...
*               PFAFF = PFAFF * (ONE - WORK( I ) * TEMP)
               LOGPF = LOGPF + dlog( cdabs(ONE - WORK( I ) * TEMP) )
               PhasePF = PhasePF * (ONE - WORK( I ) * TEMP) 
     $                       / cdabs(ONE - WORK( I ) * TEMP)
 30         CONTINUE
...
*               PFAFF = PFAFF * (-RWORK( I ))
               LOGPF = LOGPF + dlog(dabs(-RWORK( I )))
               PHASEPF = PHASEPF * dcmplx((-RWORK( I ))
     $                           / dabs(-RWORK( I )))
