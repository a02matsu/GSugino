...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!subroutine Prod_DdagD_vec(DdagD_vec, vec, Vsize,UMAT,Phimat)
!implicit none
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!subroutine Prod_Dirac_vec(D_vec, vec, Vsize,UMAT,PhiMat)
!implicit none
!
!integer, intent(in) :: Vsize !! vecsize must be sizeD
...
do f=1,num_necessary_faces
  do l_place=1,links_in_f(f)%num_
    ...
    if( f <= num_faces ) then 
      ...
      DF_chi(:,:,f)=DF_chi(:,:,f) &
        + overall_factor * dcmplx(alpha_f(f)*beta_f(f)) &
        * dir_factor * tmpmat1
    endif
