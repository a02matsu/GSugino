...
do s=1,num_sites
  ...
  do k=1,linktip_from_s(s)%num_
    ...
    do a=1,dimG
      D_vec(site_index(a,s))=D_vec(site_index(a,s)) &
        + (0d0,1d0) * dcmplx(alpha_l(l)) * lambda_ele(a,l) * overall_factor
    enddo
  enddo
enddo
...
do l=1,num_links
  ...
  do a=1,dimG
    D_vec(link_index(a,l))=D_vec(link_index(a,l)) &
      - (0d0,1d0) * dcmplx(alpha_l(l)) * eta_ele(a,s) * overall_factor
  enddo
enddo
endif
...
do l=1,num_links
  ...
  do a=1,dimG
    ...
    D_vec(link_index(a,l))=D_vec(link_index(a,l)) &
      + dcmplx(alpha_l(l)) * trace * overall_factor
  enddo
enddo
endif
...
do f=1,num_faces
  ...
  do r=1,NZF
    ...
    D_vec(face_index(a,f)) = D_vec(face_index(a,f)) &
      + (0d0,-2d0) * dcmplx(alpha_f(f)) * NZF_value(r) &
        *Phi(b,s) * chi_ele(c,f) * overall_factor
  enddo
enddo
endif
