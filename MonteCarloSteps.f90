...
complex(kind(0d0)) :: PF(1:sizeD),PF_bak(1:sizeD)
complex(kind(0d0)) :: PFsite(1:NMAT,1:NMAT,1:num_sites),PFsite_bak(1:NMAT,1:NMAT,1:num_sites)
complex(kind(0d0)) :: PFlink(1:NMAT,1:NMAT,1:num_links),PFlink_bak(1:NMAT,1:NMAT,1:num_links)
complex(kind(0d0)) :: PFface(1:NMAT,1:NMAT,1:num_faces),PFface_bak(1:NMAT,1:NMAT,1:num_faces)
...
!  do i=1,NMAT
!    do j=1,NMAT
!      tmp=tmp+PhiMat(i,j,s)*conjg(PhiMat(i,j,s))
!    enddo
!  enddo
!  write(*,*) MYRANK,s,tmp
!enddo
...
!call MPI_BCAST(UMAT,NMAT*NMAT*num_links,MPI_DOUBLE_COMPLEX,1,MPI_COMM_WORLD,IERR)
!call MPI_BCAST(PhiMat,NMAT*NMAT*num_sites,MPI_DOUBLE_COMPLEX,1,MPI_COMM_WORLD,IERR)
!call MPI_BCAST(P_AMAT,NMAT*NMAT*num_links,MPI_DOUBLE_COMPLEX,1,MPI_COMM_WORLD,IERR)
!call MPI_BCAST(P_PhiMat,NMAT*NMAT*num_sites,MPI_DOUBLE_COMPLEX,1,MPI_COMM_WORLD,IERR)
!call MPI_BCAST(UMAT,NMAT*NMAT*num_links,MPI_DOUBLE_COMPLEX,1,MPI_COMM_WORLD,IERR)
!if(MYRANK==0) then
!write(*,*) PF-PF_BAK
!endif
...
!! metropolice
write(*,'(I5,e20.10,3f10.5)') Ntau, dabs(Hnew-Hold),diff!, Hold, Hnew
...
call calc_matrix_rational_power(&
  ...
  Remez_alpha8, Remez_beta8, UMAT,PhiMat, prod_DdagD_mat, &
