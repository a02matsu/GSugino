...
subroutine calc_Qexact_Sf_face2(Sf_face,Glambda_chi,Umat)
...
do lf=1,num_faces
  do l_place=1,links_in_f(lf)%num_
    if( m_omega==-1 ) then 
      call Qexact_fermionic_face_lagrangian_adm(Lff,lf,l_place,Glambda_chi,Umat)
    elseif( m_omega==0 ) then
      call Qexact_fermionic_face_lagrangian_m0(Lff,lf,l_place,Glambda_chi,Umat)
    else
      call Qexact_fermionic_face_lagrangian(Lff,lf,l_place,Glambda_chi,Umat)
    endif
...
end subroutine calc_Qexact_Sf_face2
