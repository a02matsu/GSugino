...
"Re:A_IZ", &  ! obs(3)
"Re:Sb*A_IZ", &  ! obs(4)
"Re:mass contribution", &  ! obs(5)
"Re:correction", &  ! obs(6)
"Im:A_IZ", &  ! obs(7)
"Im:Sb*A_IZ", &  ! obs(8)
"Im:mass contribution", &  ! obs(9)
"Im:correction", &  ! obs(10)
"arg(A_IZ)"/  ! obs(10)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!! prepare dirac inverse
  ...
  WT=cmplx(dble(-dimG*(num_sites+num_links))/dble(2)) * A_IZ  &
    + cmplx(Sb)*A_IZ &
    + pcsc_mass*A_IZ &
    + pcsc_corr &
    + corr
...
!"Re:correction", &  ! obs(4)
  ...
  !!!!
  obs(3)=dble(-dimG*(num_sites+num_links))/dble(2)*dble(A_IZ) 
  obs(4)=Sb*dble(A_IZ)
  obs(5)=dble(pcsc_mass*A_IZ+pcsc_corr)
  obs(6)=dble(corr) 
  !!!!
  obs(7)=-dble(dimG*(num_sites+num_links))/dble(2)*dble((0d0,-1d0)*A_IZ) 
  obs(8)=Sb * dble((0d0,-1d0) * A_IZ)
  obs(9)=dble((0d0,-1d0) * (pcsc_mass*A_IZ+pcsc_corr)) 
  obs(10)=dble((0d0,-1d0) * corr) 
  obs(11)=arg(A_IZ)
...
do l=1,num_links
  do k=0, dimG/2
      !write(*,*) k, g_combination(-r_num,k), c_power( Bbose(l), -r_num - dble(k)) , c_power( Bfermi(l), dble(k)) 
    comp = comp & 
...
do l=1,num_links
  ...
  enddo
enddo
corr=corr * cmplx( - r_num / dble(2*num_links) )
