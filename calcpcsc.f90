...
data obs_name/ &
"Sb",&                  ! obs(1)
"Re( mu^2/2g^2 \Sigma*Tr(Phi \eta))",& ! obs(2)
"Im( mu^2/2g^2 \Sigma*Tr(Phi \eta))",& ! obs(3)
"|C_naive|", &                ! obs(4)
"arg(C_naive)", &             ! obs(5)
"|C_trace|", &                ! obs(6)
"arg(C_trace)", &             ! obs(7)
...
! PCSC
  ...
  OBS(2)=dble(pcsc_mass)
  OBS(3)=dble((0d0,-1d0)*pcsc_mass)
  call calc_compensator_naive(compensator,Phi)
  OBS(4)= abs(compensator)
  OBS(5)= arg(compensator)
  call calc_compensator_trace(compensator,Phi)
  OBS(6)= abs(compensator)
  OBS(7)= arg(compensator)
  call calc_compensator_det(compensator,Phi)
  OBS(8)= abs(compensator)
  OBS(9)= arg(compensator)
